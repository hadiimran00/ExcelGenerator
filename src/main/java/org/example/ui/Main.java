...
import com.fasterxml.jackson.databind.ObjectMapper;
import org.apache.logging.log4j.Logger;
import org.example.ui.models.UserConfig;
import org.example.ui.pages.loginPage;
import org.example.ui.utilities.*;
import org.openqa.selenium.*;
import org.openqa.selenium.support.ui.ExpectedConditions;
...
public class Main {
    private static final Logger logger = LoggerUtil.getLogger(Main.class);
    private static String resourceFolder;
    public static UserConfig currentUser;
    ...
    public static String getResourceFile(String fileName) {
        return Paths.get(resourceFolder, fileName).toAbsolutePath().toString();
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (PrintWriter writer = new PrintWriter("summary.txt")) {
            writer.print(""); // Clear summary file
        }

        List<UserConfig> users = new ObjectMapper().readValue(
                new File("users.json"), new TypeReference<>() {}
        );

        for (UserConfig user : users) {
            processUserExecution(user);
        }
    ...
    /**
     * Handles execution life-cycle for a single UserConfig.
     */
    private static void processUserExecution(UserConfig user) {
        WebDriver driver = null;
        boolean testExecuted = false;

        if (!user.shouldExecute()) {
            logger.info(" ");
            logger.info("⏩ Skipping user: {} (execute= NO)", user.getUsername());
            return;
        }

        try {
            currentUser = user;
            Properties props = DriverFactory.loadProperties();
            String downloadDir = DriverFactory.getDownloadDirectory(props);

            // Initializing driver via Factory
            driver = DriverFactory.createDriver(props, downloadDir);

            loginUser(driver, user);

            logger.info("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^");
            logger.info("🔍 Running Tests for User: {} | Country: {}", user.getUsername(), user.getCountry());
            logger.info("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^");

            List<Map<String, Object>> screens = ExcelLoader.loadScreens(user.getResolvedConfigPath());
            testExecuted = executeUserScreens(driver, screens, user.getResourcesFolder(), downloadDir);

        } catch (Exception e) {
            logger.error("💥 Unexpected error for user: {} | Message: {}", user.getUsername(), e.getMessage(), e);
            if (driver != null) {
                takeScreenshot(driver, user.getUsername() != null ? user.getUsername() : "Error");
            }
        } finally {
            cleanupUserSession(driver, user.getCountry(), testExecuted);
        }
    }

    private static boolean executeUserScreens(WebDriver driver, List<Map<String, Object>> screens, String resourcesFolder, String downloadDir) throws Exception {
        ...
        for (Map<String, Object> screen : screens) {
            ...
            if (!execute.isBlank() && !"y".equalsIgnoreCase(execute)) {
                logger.info("⏩ Skipping screen: {} (execute={})", screenName, execute);
                continue;
            }

            executedAny = true;
            executeSingleScreen(driver, screen, screens, resourcesFolder, downloadDir);
        }

        return executedAny;
    }

    private static void executeSingleScreen(WebDriver driver, Map<String, Object> screen, List<Map<String, Object>> screens, String resourcesFolder, String downloadDir) throws Exception {
        String screenName = (String) screen.get("screenName");
        String screenId = (String) screen.get("screenId");
        String mode = (String) screen.get("mode");
        String templatePath = (String) screen.get("templatePath");
        ...
        String rootPath = Paths.get(System.getProperty("user.dir"), resourcesFolder).toString();
        FileManager.setResourceFolder(rootPath);
        templatePath = Paths.get(rootPath, templatePath).toString();

        @SuppressWarnings("unchecked")
        Map<String, Map<String, Object>> validations = (Map<String, Map<String, Object>>) screen.getOrDefault("validations", Collections.emptyMap());
        @SuppressWarnings("unchecked")
        Map<String, String> scenarioData = (Map<String, String>) screen.getOrDefault("scenarioData", Map.of());
        @SuppressWarnings("unchecked")
        Map<String, Object> params = (Map<String, Object>) screen.get("params");
        @SuppressWarnings("unchecked")
        Map<String, Map<String, Object>> rules = (Map<String, Map<String, Object>>) screen.get("rules");
        ...
        Map<String, String> stringParams = new LinkedHashMap<>();
        if (params != null) {
            stringParams = params.entrySet().stream().collect(Collectors.toMap(
                    Map.Entry::getKey, e -> String.valueOf(e.getValue()), (oldVal, newVal) -> oldVal, LinkedHashMap::new
            ));
        }

        handleMenuNavigation(driver);
        ...
        ModeExecutionHandler.executeMode(
                driver, mode, screenName, templatePath, downloadDir, validateMode, testID,
                stringParams, validations, scenarioData, rules, screens, resourcesFolder
        );
    ...
    private static void handleMenuNavigation(WebDriver driver) {
        waitForLoaderToDisappear(driver);
        WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(5));
        ...
        try {
            ...
            try {
                ...
                takeScreenshot(driver, "Menu_Timeout");
                throw e2;
            }
        }
    }

    private static void loginUser(WebDriver driver, UserConfig user) {
        driver.get(user.getUrl());
        loginPage loginPage = new loginPage(driver);
        loginPage.login(user.getUsername(), user.getPassword(), user.getOrga(), user.getDist());
    }

    private static void cleanupUserSession(WebDriver driver, String country, boolean testExecuted) {
        try {
            if (country != null && testExecuted) {
                writeTestSummary(country);
                resetTestCounter();
